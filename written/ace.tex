...
%
Therefore, it is essential to know whether these effects need to be incorporated for the prediction of kinase inhibitor binding affinities.
...
%
For instance, we could compare the GAFF~\autocite{Wang2004a}, CGENFF~\autocite{Vanommeslaeghe2010a,Yu2012a}, and OPLS~\autocite{Schroedinger2014b} force field models to get an estimate of the force field error.
...
%
In case we cannot obtain reasonable \pKa estimates, we may attempt to use experimental \pKa data directly in our simulations.
...
\begin{align}
 g_n = - \ln \int \dif x_1 \dots \dif x_n \dif x_R \, e^{-u_n(x_1,\dots,x_n,x_R)} \quad,
\end{align}
%
...
%
\begin{align}
 Q([\mathrm{L}]) = 1 + K_1[\mathrm{L}] + K_1K_2[\mathrm{L}]^2 + \cdots + \left(\prod\limits_{i=1}^{n} K_i\right) [\mathrm{L}]^n = 1 + \sum\limits_{l=1}^n \left(\prod\limits_{i=1}^{l} K_l\right)[\mathrm{L}]^l \quad,
...
\begin{align}
K_{n+1} = \frac{[\mathrm{RL}_{n+1}]}{[\mathrm{RL}_n][\mathrm{L}]} = e^{-(g_{n+1}-g_n)} \quad ; g_{n+1} - g_n = - \ln (K_{n+1})
\label{eq:K_eq}
\end{align}
%
