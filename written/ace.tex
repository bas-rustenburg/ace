...
%
Our proposed framework will create an alchemical ladder from 0$\dots N_\mathrm{max}$ ligands, and can provide absolute binding free energies by using a semigrand canonical ensemble methodology~\autocite{Kofke1988a,Kofke1999a,Lynch2000a}.
%
...
As a model system, we will study the binding of several nonsteroidal anti-inflammatory drugs (NSAIDs) known to bind the pharmacologically relevant protein human serum albumin (HSA) (\Cref{figure:hsa-compounds})~\autocite{Zsila2011a}.
%
...
Where the term in the integral can actually be replaced by the binding polynomial, 
...
\end{align}
In this equation, $K_n$ are the stoichiometric binding constants, not site constants (see also \Cref{figure:semigrand}) meaning we do not define a specific site.
%
\begin{align}
...
%
By knowing the free energies of each occupation state, we can then estimate the bound concentrations for a given concentration of receptor $R$ and ligand $L$.
...
%
This means that we can also perform ITC experiments as a validation of our calculations.
%
...
%
This is too primitive to actually describe multiple binding events, and contains errors that are made by concentrations.
%
...
%
We will extend our Bayesian analysis tools with the capacity to select between possible association models without limiting the number of distinguishable sites or binding stoichiometry.
%
...
%
By sampling the correlated uncertainty, we improve over fits as provided by SEDPHAT, that contain more parameters, but do not properly account for correlated uncertainties.
