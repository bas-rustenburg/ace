...
%
However, they form a potential pitfall when modeling, since these unknown contributions potentially lead to large errors in the binding affinity (\Cref{figure:pka-kinase}).
...
%
The compounds we have selected are known to bind to various sites on HSA~\autocite{Zsila2011a}, and are soluble enough to perform ITC experiments for validation.
%
...
%
From the estimated enthalpy and association constants we can simulate an ITC experiment, where we calculate the complex concentrations as a function of the receptor and ligand concentration, as heats $q_m$ is liberated with each injection $m$
%
\begin{align}
...
In order to validate the results obtained from free energy calculations, we will perform complementary ITC experiments on HSA. 
%
...
%
Unlike a Bayesian analysis, it does not propagate errors accurately and just provides a ``fudge factor'' to improve the quality of the fit.
%
...
\subsubsection*{Pitfalls and alternatives}
Our simulations may suffer from sampling issues in our free energy calculations.
%
...
%
We may need to employ some methods that drive the simulation to sample more states of the system.
%
